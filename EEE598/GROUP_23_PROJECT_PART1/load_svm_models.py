...
def SVM_fit(model1,trainloader, testloader,size,flag):
    ...
    if flag ==0:
        clf = pickle.load(open('VGG16_SVM.pkl', 'rb'))
        title = "VGG16"
    else:
        clf = pickle.load(open('Resnet18_SVM.pkl', 'rb'))
        title = "Resnet18"
    ...
    #clf.fit(features, targets) #trainloader.dataset.train_labels)
    ...
    for i, (in_data, target) in enumerate(testloader):
        ...
        if flag == 1: feats_test = feats_test.view(16,512)
        y_pred += list(clf.predict(feats_test.data.numpy()))#.to_list()
...
#VGG_SVM = pickle.load(open('VGG16_SVM.pkl', 'rb'))
...
#Resnet_SVM = pickle.load(open('Resnet18_SVM.pkl', 'rb'))
